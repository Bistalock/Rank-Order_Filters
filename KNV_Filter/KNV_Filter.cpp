			...
			for(int j = offset; j < maxRow - offset; j++) {
				...
				for(currentX; currentX < rightCol; currentX++) {
					for(currentY; currentY < bottomRow; currentY++) {
						currentKernelValues[currentX][currentY] = img[i][currentX][currentY]; //** ERROR: cannot assign currentKernelvalues[] from img[][][]. is it currentKernelValues[][]?
					}
				}
				...
				currentKernelValues[i][k][j] = abs(currentKernelValues[j][k] - median); // Notice the appearance of j and k being backwards, but k and j correspond to x and y, respectively.
				}
			}
		}
...
unsigned char getMedian(unsigned char** currentKernalValues) {
    int elements = sizeof(currentKernalValues) / sizeof(currentKernalValues[0][0]);
